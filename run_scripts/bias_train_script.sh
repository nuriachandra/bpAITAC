...
# [17] 'protein_free' if true we use the protein free data, if not we used data from closed regions
...
# info_file=$info_file_dir"complete_bias_corrected_normalized_4.3.23-2/memmap/info.txt"
...
then
    ...
    # Set different info directory based on protein_free parameter
    if [ "$protein_free" == "protein_free" ]
    then
        chelan_info_dir="/data/nchand/ImmGen/mouse/bias/selectedBACs_gt6avgcount_N500000bp_filtered_11.2.24/"
    else
        chelan_info_dir="/data/nchand/ImmGen/mouse/bias/tn5_bias_counts250bp_combined_filtered_11.2.24/"
    fi

    data_dir=$chelan_data_dir
    info_file_dir=$chelan_info_dir
    info_file=$info_file_dir"bias_config.yaml"
    results_dir="/homes/gws/nchand/MostafaviLab/results/"
    celltypes=$info_file_dir"celltype_names_standin.npy"
fi
...
echo "Is protein-free: ${protein_free}"
...
if [ "$trial_type" = "sample" ]; then
    ...
    then
        ...
        #  for 250 bp example
        # seq_len=250
        # info_file="/data/nchand/ImmGen/mouse/bias/selectedBACs_gt6avgcount_N500000bp250_11.2.24_sample/bias_config.yaml"
        info_file="/data/nchand/ImmGen/mouse/bias/selectedBACs_gt6avgcount_N500000bp_filtered_11.1.24_sample/bias_config.yaml"
    else
        # first 2000 datapoints
        info_file="/data/nchand/ImmGen/mouse/bias/tn5_bias_counts250bp_combined_prepped_10.24.24_sample/bias_config.yaml" 
    fi

    python train.py $info_file $celltypes $seq_len $name $model $output_dir $loss_fxn $num_epochs $lambda $bias $n_filters $ocr_eval $bin_size $bin_pooling_type $scalar_head_fc_layers $batch_size $learning_rate $lr_scheduler_name "$lr_scheduler_args" $set_seed $memmaped_data --save_best_loss_model #> $error_dir 2>&1
