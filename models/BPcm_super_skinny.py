...
class BPcm_super_skinny(nn.Module):
    """
    ...
    def __init__(self, seq_len:int, num_filters:int=300, n_celltypes:int=90, bin_size:int = 1, bin_pooling_type:nn.Module=nn.MaxPool1d, scalar_head_fc_layers:int=1):
        super(BPcm_super_skinny, self).__init__()

        self.num_filters = num_filters
        n_body_layers = 0 # this is different from BPcm
...
class BPcmHead(nn.Module):
    ...
    def __init__(self, seq_length:int, n_celltypes:int, input_channels:int, pool_size:List[int], n_reps:int, conv_width:int, bin_size:int, scalar_head_fc_layers:int, scalar_head_hidden_layer:int, profile_out_len:int= 998):
        """
