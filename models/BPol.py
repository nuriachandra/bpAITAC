...
from modules import Body, ProfileHead, ScalarHeadMultiMaxpool, ScalarHeadConvMaxpool, ScalarHeadOneLayer, ScalarHeadMaxPool
from typing import List # use to specify list type

class BPol(nn.Module):
    """
    ...
    """
    def __init__(self, seq_len:int, num_filters:int=300, n_celltypes:int=90, bin_size:int = 1, bin_pooling_type:nn.Module=nn.MaxPool1d):
        super(BPol, self).__init__()
        ...
        self.head = BPolHead(input_channels=num_filters,
                                           seq_length=seq_len, 
...
class BPolHead(nn.Module):
    ...
    def __init__(self, seq_length:int, n_celltypes:int, input_channels:int, pool_size:List[int], n_reps:int, conv_width:int, bin_size:int, profile_out_len:int=998):
        ...
        self.total_count_prediction = ScalarHeadOneLayer(in_channels=in_channels,
                                                            seq_len=seq_length, 
