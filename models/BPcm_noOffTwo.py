...
class BPcm_noOffTwo(nn.Module):
    ...
    def __init__(self, seq_len:int, num_filters:int=300, n_celltypes:int=90, bin_size:int = 1, bin_pooling_type:nn.Module=nn.MaxPool1d, scalar_head_fc_layers:int=1):
        super(BPcm_noOffTwo, self).__init__()
...
class BPcmHead(nn.Module):
    ...
    def forward(self, x, bias):
        ...
        """
        zeros = torch.zeros(bias.shape[0], 2, device=bias.device)
    
        # Concatenate zeros and original tensor along last dimension
        padded = torch.cat([zeros, bias], dim=1)
        
        # Remove last two elements along last dimension
        bias_adjusted = padded[:, :-2]

        profile = self.profile_prediction(x, bias_adjusted)
        if torch.isnan(torch.mean(profile)):
            print("IN BPcm Head THE PROFILE HAS AN NAN")
            print(profile)
            print(profile.size())
        
        scalar = self.total_count_prediction(x, bias_adjusted)
