...
class BPbi_shallow(nn.Module):
    """
    This model uses BPnetRep body, but with only 2 dialated conv layers with 300 filters (as the default)
    ...
    def __init__(self, seq_len:int, num_filters:int=300, n_celltypes:int=90, bin_size:int = 1, bin_pooling_type:nn.Module=nn.MaxPool1d, scalar_head_fc_layers:int=1):
        super(BPbi_shallow, self).__init__()
        ...
        conv_width = 3
        n_dialated_conv_layers = 2
        scalar_head_hidden_layer = 600
        self.body = Body(n_filters=self.num_filters, num_dialated_layers=n_dialated_conv_layers) 
    ...
    def forward(self, input, bias):
        ...
        x = self.body(input)
        # print('BPbi after body', x.size())
        if (torch.isnan(torch.mean(x))):
            print('After Body in BPcm is nan')
        footprint, scalar = self.head(x, bias)
        # print('BPbi footprint and scalar', footprint.size(), scalar.size())
        return footprint, scalar
    
    def predict_logits(self, input):
        # check if data is nan
        # print('input in pred logits bpbi is', input.size())
        # if (torch.isnan(torch.mean(input))):
        #     print('input to BPcm is nan')
        # print('input to predict logits, ', input)
        # print('shape of input to body in bpbi', input.size())
        x = self.body(input)
        # print('BPbi after body', x.size())
        if (torch.isnan(torch.mean(x))):
            print('After Body in BPcm is nan')
        logits = self.head.predict_logits(x)
        # print('BPbi footprint and scalar', footprint.size(), scalar.size())
        return logits
...
class BPcmHead(nn.Module):
    ...
    def __init__(self, seq_length:int, n_celltypes:int, input_channels:int, pool_size:List[int], n_reps:int, conv_width:int, bin_size:int, scalar_head_fc_layers:int, scalar_head_hidden_layer:int):
        ...
        self.profile_prediction = ProfileHead(in_channels, n_celltypes, bin_size=bin_size, profile_out_len=250)
    ...
    def predict_logits(self, x):
        logits = self.profile_prediction.predict_logits(x)
        return logits
