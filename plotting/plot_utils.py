...
def plot_loss_file(input_file, output_file_path, loss_type):
    # load the file 
    loss = np.loadtxt(input_file)
    plot_training_loss(loss, output_file_path, loss_name)

def plot_training_loss(training_loss, output_file_path, loss_name):
    #plot line plot of loss function per epoch
    plt.figure(figsize=(16, 9))
    plt.plot(np.arange(len(training_loss)), training_loss)
    plt.xlabel('Epoch', fontsize=18)
    plt.ylabel('Loss', fontsize=18)
    plt.savefig(output_file_path + "/" + name + ".png")
    plt.close()


def plot_cors(obs, pred, output_file_path):
    "obs and pred are numpy arrays"
    correlations = []
    vars = []
    ...
    plt.savefig(output_file_path + "cor_hist.png")
    plt.close()
...
def plot_corr_variance(labels, correlations, output_file_path):
    ...
    plt.savefig(output_file_path + "variance_correlation_plot.png")
    plt.close()
    ...
    plt.ylabel('Prediction-ground truth correlation', fontsize=18)
    plt.colorbar()
    plt.savefig(output_file_path + "variance_correlation_hist2D.png")
    plt.close()
    ...
    plt.ylabel('Prediction-ground truth correlation', fontsize=18)
    plt.colorbar()
    plt.savefig(output_file_path + "variance_correlation_loghist2d.png")
    plt.close()
